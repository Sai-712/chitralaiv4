...
const AttendeeDashboard: React.FC = () => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (attendeeImageData.length > 0) {
          ...
          // Check if the user has a default selfie even if they have no events
          ...
          // No event data found, set empty states
          setAttendedEvents([]);
          setMatchingImages([]);
          setFilteredImages([]);
        }
  ...
  const handleSelfieChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      // Set the selfie file and preview
      setSelfie(file);
  ...
  const handleUploadAndCompare = async () => {
    ...
    try {
      ...
      // Use complete event details for display and database
      const eventName = completeEventDetails.name;
      const eventDate = completeEventDetails.date;
      ...
      // Add this event to attended events if not already there
      const eventExists = attendedEvents.some(event => event.eventId === eventDetails.id);
      ...
      if (!eventExists) {
        const newEvent: Event = {
          ...
          eventDate: eventDate,
          // Use event's coverImage if available, otherwise fall back to first matched image
          thumbnailUrl: completeEventDetails.coverImage || sortedMatches[0].url,
      ...
      // Set success message and filter to show only this event's images
      setSuccessMessage(`Found ${sortedMatches.length} new photos from ${eventName}!`);
      setSelectedEventFilter(eventDetails.id);
      ...
      console.error('Error in upload and comparison:', error);
      setError(error.message || 'Error processing your request. Please try again.');
    } finally {
      setIsUploading(false);
    }
                ...
                <div className="border border-blue-200 bg-blue-50 p-3 rounded-lg mb-3">
                  ...
                  <div className="mt-3">
                    ...
                    {selfiePreview ? (
                      ...
                      <label className="cursor-pointer bg-blue-100 text-blue-800 px-3 py-1.5 rounded-lg hover:bg-blue-200 transition-colors inline-block text-sm">
                  ...
                  {attendedEvents.map((event) => (
                    <div
                      ...
                      className={`bg-white rounded-lg border overflow-hidden hover:shadow-md transition-shadow cursor-pointer ${
                        selectedEventFilter === event.eventId ? 'border-blue-500 ring-2 ring-blue-200' : 'border-gray-200'
                      }`}
                    ...
                    >
                      ...
                      <div className="p-2">
                        ...
                        <div className="flex justify-between items-center mt-1">
                          <p className="text-xs text-gray-600">{new Date(event.eventDate).toLocaleDateString()}</p>
